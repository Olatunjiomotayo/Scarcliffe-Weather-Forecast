getwd()
setwd("C:\\Users\\olatu\\OneDrive\\Documents\\DAT7006\\ASS 2")
...
hist(Scarcliffenew_df$TSLB, main = "Histogram of Soil Temperature Q2", xlab = "Soil Temperature in Celsius", col = "yellow")
...
# To determine the RAINC central tendency and distribution, Summary() function 
# in R will give us the basic descriptive statistical analysis such as the minimum 
# value, 1st quartile, Mean, Median, 3rd quartile and the Maximum number. 

#Histogram will also be used to visualise RAINC frequency distribution to understand its 
# distribution shape

# Additionally, I will use Shapiro-Wilk to test the normality of RAINC to determine 
# if it follow a normal distribution.
...
# The Shapiro-Wilk test results show that RAINC deviates from a normal distribution. 
# It is very obvious to arrive at this mere looking at the histogram. 


# BIRAVARIATE ANALYSIS

# Let's explore the correlation between temperature and Soil Temperature in Scarcliffe, Derbyshire, by plotting a scatter plot.
...
# Pearson correlation test shows the direction and strength of linear connection 
# between surface temperature and soil temperature. From the result, it means 
# surface temperature and soil temperature have strong correlation in the positive direction.


# MULTIVARIATE ANALYSIS

# To investigate the correlations between a dependent variable, surface temperature (TSK) 
# and several independent variables such as, surface pressure PSFC, humidity Q2, 
# precipitation RAINC, soil temperature TSLB, and wind speed Windspeed), we can 
# design a multiple linear regression model.
...
# This study provides insight into the individual and combined impacts of the independent 
# variables on the dependent variable, as well as the overall model fit and significance.

# Basicly, what this regression summary means is that both TSLB and Q2 are strong predictor
# to our dependant variable, TSK, also windspeed is a strong predictor too, but not as strong 
# as TSLB and Q2, other variables, PSFC and RAINC have no significance at all.
...
# p-value is 4.995e-07, which means the normality of errors is not normally distributed,
# The p-value of 4.995e-07 is exceedingly small, significantly below the standard 
# significance value of 0.05. This means that there is strong evidence to reject the null hypothesis of normality.



#To check for no autocorrelation of errors 
#Durbin-Watson test

#null hypo: there is no autocorrelation (errors are independent)

dwtest(mlr_model) #since p>0.05 - there is no autocorrelation

#p-value is 2.2e-16 which is less than 0.05, this simply means there is
...
# and Windspeed) are less than 10, indicating that the model lacks substantial 
# multicollinearity. Multicollinearity is the condition in which two or more predictor 
# variables are highly linked, resulting in unstable and incorrect coefficient estimations. 
